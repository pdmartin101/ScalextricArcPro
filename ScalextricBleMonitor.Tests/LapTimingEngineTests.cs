...
public class LapTimingEngineTests
{
    ...
    [Fact]
    public void UpdateTimestamps_FirstTimestamp_EstablishesBaseline()
    {
        var engine = new LapTimingEngine();
    ...
    [Fact]
    public void UpdateTimestamps_SecondCrossing_StartsLap1()
    {
        var engine = new LapTimingEngine();

        // First crossing establishes baseline
        engine.UpdateTimestamps(1000, 0);

        // Second crossing starts lap 1
        var result = engine.UpdateTimestamps(2000, 0);

        Assert.True(result.LapCompleted);
        Assert.Equal(1, result.CurrentLap);
        Assert.Equal(1, engine.CurrentLap);
        Assert.Equal(0, result.LapTimeSeconds); // No lap time for first crossing
    }

    [Fact]
    public void UpdateTimestamps_ThirdCrossing_CompletesLap1()
    {
        var engine = new LapTimingEngine();
    ...
    public void UpdateTimestamps_FasterLap_UpdatesBestTime()
    {
        ...
        // Setup: baseline, start lap 1, complete lap 1 (10s)
        engine.UpdateTimestamps(1000, 0);
        engine.UpdateTimestamps(2000, 0);
        engine.UpdateTimestamps(3000, 0);

        // Complete lap 2 in 8 seconds (faster)
    ...
    public void UpdateTimestamps_SlowerLap_KeepsBestTime()
    {
        ...
        // Complete lap 2 in 12 seconds (slower)
    ...
    public void UpdateTimestamps_Lane2Crossing_DetectsCorrectLane()
    {
        ...
        // Cross on lane 2 (higher timestamp)
    ...
    public void UpdateTimestamps_Lane1Crossing_DetectsCorrectLane()
    {
        ...
        // Cross on lane 1 (lane 1 has higher timestamp)
    ...
    public void Reset_ClearsAllState()
    {
        ...
        // After reset, next timestamp should be treated as baseline again
    ...
    public void UpdateTimestamps_TimestampOverflow_CalculatesCorrectLapTime()
    {
        ...
        // Setup near uint max value
        engine.UpdateTimestamps(uint.MaxValue - 500, 0);
        engine.UpdateTimestamps(uint.MaxValue - 400, 0);

        // Overflow scenario: timestamp wraps around
        var result = engine.UpdateTimestamps(100, 0);

        Assert.True(result.LapCompleted);
        // Time difference should be 500 (400 remaining + 100 after wrap + 1 for the wrap)
    ...
    public void NoChange_ReturnsCorrectState()
    {
        ...
        engine.UpdateTimestamps(1000, 0);
        engine.UpdateTimestamps(2000, 0);
        engine.UpdateTimestamps(3000, 0); // Sets best lap to 10s
