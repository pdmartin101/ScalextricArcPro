...
public class PowerHeartbeatService : IPowerHeartbeatService
{
    ...
    public PowerHeartbeatService(IBleService bleService)
    {
    ...
    public async Task SendPowerOffSequenceAsync()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            await _bleService.WriteCharacteristicAsync(ScalextricProtocol.Characteristics.Command, clearGhostCommand);
        ...
        for (int i = 0; i < 3; i++)
        {
            await _bleService.WriteCharacteristicAsync(ScalextricProtocol.Characteristics.Command, powerOffCommand);
            await Task.Delay(BleWriteDelayMs);
        }
    }
